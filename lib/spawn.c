...
spawn(const char *prog, const char **argv)
{
	...
	child_tf.user_ip = elf->e_entry;

	if ((r = init_stack(child, argv, &child_tf.user_sp)) < 0)
		return r;
	...
	for (i = 0; i < elf->e_phnum; i++, ph++) {
		...
		if (ph->p_flags & ELF_PROG_FLAG_WRITE)
			perm = L2_TYPE_SMALL | L2_SMALL_AP_URW_SRW;
		else
			perm = L2_TYPE_SMALL | L2_SMALL_AP_URO_SRW;
...
init_stack(envid_t child, const char **argv, uintptr_t *init_esp)
{
	...
	if ((r = sys_page_alloc(0, (void*) UTEMP, L2_TYPE_SMALL | L2_SMALL_AP_URW_SRW)) < 0)
		return r;
	...
	*init_esp = UTEMP2USTACK(&argv_store[-2]);
	...
	if ((r = sys_page_map(0, UTEMP, child, (void*) (USTACKTOP - PGSIZE), L2_TYPE_SMALL | L2_SMALL_AP_URW_SRW)) < 0)
		goto error;
	...
	int fd, size_t filesz, off_t fileoffset, int perm)
{
	...
	// if va not page-aligned, read the previous
	if ((i = PGOFF(va))) {
		va -= i;
		memsz += i;
		filesz += i;
		fileoffset -= i; // 不用考虑offset < 0的情况吗？
	}

	// memsz >= filesz, because memsz = filesz + bss
	for (i = 0; i < memsz; i += PGSIZE) {
		if (i >= filesz) {
			...
			// from file
			if ((r = sys_page_alloc(0, UTEMP, L2_TYPE_SMALL | L2_SMALL_AP_URW_SRW)) < 0)
				return r;
...
{
	int r;
	for(uint32_t i = L1X(UTEXT); i < L1X(UXSTACKTOP); i++){
		if(sys_get_l1(LADDR(i, 0, 0)) & L1_EXIST){
			for(uint32_t j = 0; j < L2NENTRIES; j++){
				if((sys_get_l2(LADDR(i, j, 0)) & L2_EXIST) && (sys_get_l2(LADDR(i, j, 0)) & L2_SMALL_SHARED)){
					if ((r = sys_page_map(
							thisenv->env_id, (void *)LADDR(i, j, 0), 
							child, (void *)LADDR(i, j, 0), 
							sys_get_l2(LADDR(i, j, 0)) & L2_SMALL_FOR_USER)) < 0)
						return r;
				}
			}
		}
