...
openfile_alloc(struct OpenFile **o)
{
	...
	for (i = 0; i < MAXOPEN; i++) {
		// case 0表示还没有为这个Fd分配（Fd所在的）页面
		// case 1表示已分配页面，且没有用户进程占用
		// case >1表示有进程占用，不能分配
		switch (pageref(opentab[i].o_fd)) {
		case 0:
			// opentab[i].o_fd是0xD0000000+i*PGSIZE
			if ((r = sys_page_alloc(0, opentab[i].o_fd, L2_TYPE_SMALL | L2_SMALL_AP_URW_SRW)) < 0)
				return r;
			/* fall through */
		case 1:
...
try_open:
	...
	*pg_store = o->o_fd;
	*perm_store = L2_TYPE_SMALL | L2_SMALL_AP_URW_SRW | L2_SMALL_SHARED;
...
{
	struct Fsret_write *ret = (struct Fsret_write *)req;
	struct OpenFile *o;
	int r;
	...
	req->req_n = req->req_n > max_size ? max_size : req->req_n;
